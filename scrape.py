...
# JS: among elements matching `sel` that look like a price, return the text of the
# one whose enclosing card also mentions the target denomination amount.
_MATCH_JS = r"""
(args) => {
  const {sel, amt} = args;
  const priceRe = /(US\$|USD|RM|MYR|S\$|SGD|€|£|\$)\s?\d[\d.,]*|\d[\d.,]*\s?(USD|EUR|MYR|SGD|RM)/i;
  const re = amt != null ? new RegExp('(?<!\\d)' + amt + '(?!\\d)') : null;
  const els = [...document.querySelectorAll(sel)];
  for (const pe of els) {
    const ptext = pe.textContent || '';
    if (!priceRe.test(ptext)) continue;
    if (!re) return ptext.trim();
    let node = pe;
    for (let i = 0; i < 6 && node; i++) {
      const t = (node.textContent || '').replace(/,/g, '');
      if (re.test(t) && /USD|US\$|\$/.test(node.textContent || '')) return ptext.trim();
...
# Competitors without a working adapter yet — held off so they don't add junk.
SKIP_COMPS = {"OG", "G2A", "G2G"}
